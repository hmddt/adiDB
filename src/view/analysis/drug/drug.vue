<template>
  <div class="drug">
    <div style="text-align: left ;margin-left: 60px;margin-top: 20px">
      <br>
      <p>
          <span style="font-weight: bold;font-size: 24px;color: #3a8ee6">Drug Susceptibility Analysis
          </span>
...
export default {
  ...
  methods:{
    ...
    getcheckpointPic() {
      ...
      this.loading = true;
      this.png64 = "";
      this.$http.get("/rnameapi/r/drug/" + this.gene).then(res => {
        this.png64 = res.data;
        this.$http.get('/rnameapi/data/drug/' + this.gene).then(res =>{
    ...
    downloadDiffImg(){
      ...
      // 保存 PDF 文件
      let filename = this.gene + '_drug.pdf';
      doc.save(filename);
    },
    showTooltip(event) {
