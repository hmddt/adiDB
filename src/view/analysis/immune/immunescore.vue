<template>
  <div id="immunescore">
    <div style="text-align: left ;margin-left: 60px;margin-top: 20px">
      <br>
      <p>
          <span style="font-weight: bold;font-size: 24px;color: #3a8ee6">Immune Score Analysis
          </span>
...
<!--        <el-form-item label="Gene" v-show="!allGene">-->
        ...
        <el-button type="primary" class="hover-blue-btn" @click="getimmunescorePic">Click</el-button>
      </el-form>
    </div>
    ...
    <div v-loading="loading" v-show="show"  >
      <div class="checkpoint-left" style=" height:650px;background-color: #ffffff;">
        <div class="image-container" style=" height: 600px; overflow: hidden;">
          <el-image
            style="width: 1000px; margin-top:-200px;object-fit: cover;"
        ...
        <!--免疫检查点图片图片-->
        <br>
        <el-button type="primary" round @click="downloadDiffImg" style="margin-top: -200px">Download</el-button>
      </div>

<!--      <div class="checkpoint-right" style=" position: relative; margin-left: 4%; margin-top:0;padding: 10px;width: 45%;background-color: #ffffff;">-->
<!--        &lt;!&ndash;拿到checkpoint数据&ndash;&gt;-->
<!--        <span style="font-weight: bold;font-size: 14px"> Correlation with immunescore </span>-->
<!--        <el-table-->
<!--          :stripe="true"-->
<!--          :data="score"-->
<!--          height="500"-->
...
export default {
  ...
  methods:{
    ...
    getimmunescorePic() {
      ...
      this.png64 = "";
      Promise.all([
        this.$http.get("http://47.97.192.100:8989/r/immu/immunescore/" + this.gene)
      ]).then(([imgRes, dataRes]) => {
    ...
    downloadDiffImg(){
      ...
      // 保存 PDF 文件
      let filename = this.gene + '_immuscore_image.pdf';
      doc.save(filename);
    },
    //将表格内内容下载下来
    downloadDiffDataData(){
      let headerRow = ['Gene', 'Cancer', 'score', 'Cor', 'Sig'];
      //将csv表格内内容格式化
      ...
      // 生成数据行
      let csvData = this.score.map(row => {
        // 过滤掉 id 属性，只取需要的属性值
        let { id, ...rowData } = row;
      ...
      let filename = this.gene+'_immuscore.csv'
