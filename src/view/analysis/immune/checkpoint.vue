...
<div id="cpbody">
  <div style="text-align: left ;margin-left: 60px;margin-top: 20px">
    <br>
    <p>
          <span style="font-weight: bold;font-size: 24px;color: #3a8ee6">Immune Checkpoint Analysis
          </span>
...
<!--      <el-form-item label="Gene" v-show="!allGene">-->
  ...
  <div v-loading="loading" v-show="show" class="checkpoint_show" style="position: relative;" >
    <div class="checkpoint-left" style="  background-color: #ffffff;">
      <!--免疫检查点图片图片-->
      <div class="image-container" style=" height: 750px; overflow: hidden;">
        <el-image
          style=" width:650px;margin-top: -60px"
...
<!--    <div class="checkpoint-right" style=" position: relative; margin-left: 5%; margin-top:5.5%;padding: 10px;width: 49%; background-color: #ffffff;">-->
<!--      &lt;!&ndash;拿到checkpoint数据&ndash;&gt;-->
<!--      <span style="font-weight: bold"> Correlation with checkpoints </span>-->
<!--      <el-table-->
<!--        :stripe="true"-->
<!--        :data="checkpoint"-->
<!--        height="500"-->
<!--        border-->
<!--        style="width: 100%;height: 88%;top: 20px">-->
<!--        <el-table-column-->
<!--          prop="gene"-->
<!--          label="gene">-->
<!--        </el-table-column>-->
...
export default {
  ...
  methods:{
    ...
    getcheckpointPic() {
      ...
      this.png64 = "";
      Promise.all([
        this.$http.get("http://47.97.192.100:8989/r/immu/checkpoint/" + this.gene)
      ]).then(([imgRes, dataRes]) => {
    ...
    downloadDiffImg(){
      ...
      // 保存 PDF 文件
      let filename = this.gene + '_checkpoint_image.pdf';
      doc.save(filename);
    },
    //将表格内内容下载下来
    downloadDiffDataData(){
      let headerRow = ['Gene', 'Cancer', 'Checkpoint', 'Cor', 'Sig'];
      //将csv表格内内容格式化
      ...
      // 生成数据行
      let csvData = this.checkpoint.map(row => {
        // 过滤掉 id 属性，只取需要的属性值
        let { id, ...rowData } = row;
      ...
      let filename = this.gene+'_checkpoint.csv'
    ...
    init(){
      setTimeout(()=>{
        this.loading = true;
        this.png64 = "http://tmliang.cn/pic/adi/analysis/LEP_checkpoint_image.png";
