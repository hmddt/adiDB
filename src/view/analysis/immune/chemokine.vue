...
<div id="chemokine">
  <div style="text-align: left ;margin-left: 60px;margin-top: 20px">
    <br>
    <p>
          <span style="font-weight: bold;font-size: 24px;color: #3a8ee6">Immunochemokine Analysis
          </span>
  ...
  <div style="margin: 20px; text-align: center">
    ...
    <el-button type="primary" class="hover-blue-btn" @click="getchemokinePic">Click</el-button>
    ...
    <div v-loading="loading" v-show="show" style="position: relative;" >
      <div class="checkpoint-left"  style=" background-color: #ffffff;height: 600px;margin-top: 50px">
        <!--免疫检查点图片图片-->
        <el-image
          style=" height: 550px"
        ...
        <br>
        <el-button type="primary"  style="" round @click="downloadDiffImg">Download</el-button>
      </div>

<!--      <div class="checkpoint-right" style=" position: relative; margin-left: 5%; margin-top:5%;padding: 10px;margin-bottom: 120px;width: 29%; background-color: #ffffff;">-->
<!--        &lt;!&ndash;拿到chemokine数据&ndash;&gt;-->
<!--        <span style="font-weight: bold;font-size: 14px"> Correlation with chemokine </span>-->
<!--        <el-table-->
<!--          :stripe="true"-->
<!--          :data="chemokine"-->
<!--          height="500"-->
<!--          border-->
<!--          style="width: 100%;height: 83%;top: 20px">-->
<!--          <el-table-column-->
<!--            prop="gene"-->
<!--            label="gene">-->
<!--          </el-table-column>-->
...
export default {
  ...
  methods:{
    ...
    getchemokinePic() {
      ...
      this.png64 = "";
      Promise.all([
        this.$http.get("http://47.97.192.100:8989/r/immu/chemokine/" + this.gene)
      ]).then(([imgRes, dataRes]) => {
    ...
    downloadDiffImg(){
      ...
      // 保存 PDF 文件
      let filename = this.gene + '_chemokine_image.pdf';
      doc.save(filename);
    },
    //将表格内内容下载下来
    downloadDiffDataData(){
      let headerRow = ['Gene', 'Cancer', 'Chemokine', 'Cor', 'Sig'];
      //将csv表格内内容格式化
      ...
      // 生成数据行
      let csvData = this.chemokine.map(row => {
        // 过滤掉 id 属性，只取需要的属性值
        let { id, ...rowData } = row;
      ...
      let filename = this.gene+'_chemokine.csv'
    ...
    init(){
      setTimeout(()=>{
        ...
        this.png64 = "http://tmliang.cn/pic/adi/analysis/LEP_chemokine_image.png";
