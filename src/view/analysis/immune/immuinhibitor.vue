<template>
<!--  免疫抑制因子分析 -->
  <div id="immuinhibitor">
    <div style="text-align: left ;margin-left: 60px;margin-top: 20px">
      <br>
      <p>
          <span style="font-weight: bold;font-size: 24px;color: #3a8ee6">Immune Inhibitor Factor Analysis
          </span>
...
<!--        <el-form-item label="Gene" v-show="!allGene">-->
        ...
        <el-button type="primary" class="hover-blue-btn" @click="getimmuinhibitorPic">Click</el-button>
      </el-form>
    </div>
    ...
    <div v-loading="loading" v-show="show"  >
      <div class="checkpoint-left" style=" height:750px;margin-top:25px;background-color: #ffffff;">
        <!--免疫检查点图片图片-->
        <el-image
          style=" height: 675px"
...
<!--      <div class="checkpoint-right" style=" position: relative; margin-left: 0; margin-top:6px;padding: 10px;width: 40%;background-color: #ffffff;">-->
<!--        &lt;!&ndash;拿到checkpoint数据&ndash;&gt;-->
<!--        <span style="font-weight: bold"> Correlation with immuinhibitor </span>-->
<!--        <el-table-->
<!--          :stripe="true"-->
<!--          :data="immut"-->
<!--          height="500"-->
<!--          border-->
<!--          style="width: 100%;height: 86%;top: 20px">-->
<!--          <el-table-column-->
<!--            prop="gene"-->
<!--            label="gene">-->
<!--          </el-table-column>-->
...
export default {
  ...
  methods:{
    ...
    getimmuinhibitorPic() {
      ...
      this.png64 = "";
      Promise.all([
        this.$http.get("http://47.97.192.100:8989/r/immu/immuinhibitor/" + this.gene)
      ]).then(([imgRes, dataRes]) => {
    ...
    downloadDiffImg(){
      ...
      // 保存 PDF 文件
      let filename = this.gene + '_immuinhibitor_image.pdf';
      doc.save(filename);
    },
    //将表格内内容下载下来
    downloadDiffDataData(){
      let headerRow = ['Gene', 'Cancer', 'immuinhibitor', 'Cor', 'Sig'];
      //将csv表格内内容格式化
      ...
      // 生成数据行
      let csvData = this.immut.map(row => {
        // 过滤掉 id 属性，只取需要的属性值
        let { id, ...rowData } = row;
      ...
      let filename = this.gene+'_immuinhibitor.csv'
