...
export default {
  ...
  mounted() {
    ...
    // this.$http.get('/adiapi/adi/all').then(res=>{
    ...
    // this.$http.get('/adiapi/adi/allToAbstract').then(res=>{
    //   this.tableDate = res.data;
    //   this.tablePageTotal = this.tableDate.length; // Set total items count
    //   this.updatePaginatedData(); // Initialize pagination data
  ...
  methods: {
    async fetchInitialData() {
      try {
        ...
        const geneRes = await this.$http.get('/adiapi/adi/gene');
        this.allgene = geneRes.data;  // 设置 gene 数据
        
        // 请求10条数据接口（假设你有一个返回10条数据的接口）
        const initialDataRes = await this.$http.get('/adiapi/adi/allToAbstract');
        ...
        // 请求全部数据
        const fullDataRes = await this.$http.get('/adiapi/adi/allToAbstract/full');
        // 使用全部数据更新表格内容
